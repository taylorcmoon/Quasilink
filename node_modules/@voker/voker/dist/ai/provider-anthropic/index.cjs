...
// src/ai/provider-anthropic/index.ts
var provider_anthropic_exports = {};
__export(provider_anthropic_exports, {
  Anthropic: () => Anthropic2
});
module.exports = __toCommonJS(provider_anthropic_exports);
...
var ApiClient_Fingerprints = class extends ApiClient_BaseSubPath {
  ...
  async create() {
    ...
    try {
      const rootPkgPath = import_path8.default.resolve(process.cwd(), "package.json");
...
// src/ai/provider-anthropic/index.ts
var Anthropic2 = class extends Anthropic {
  vokerClient;
  constructor(config = {}) {
    const { vokerClient, ...anthropicConfig } = config;
    ...
    this.messages = new Messages3(this, this.vokerClient);
    this.beta = new Beta2(this, this.vokerClient);
  }
};
var Messages3 = class extends Anthropic.Messages {
  constructor(baseAnthropic, vokerClient) {
    super(baseAnthropic);
    this.vokerClient = vokerClient;
  }
  create(body, options) {
    ...
    const messageRequest = super.create(restParams, options);
    if (body.stream === false || body.stream === null || body.stream === void 0) {
      return messageRequest.then((message) => {
    ...
    return messageRequest.then((response) => {
      if (!("tee" in response)) {
        return response;
      }
      const [stream1, stream2] = response.tee();
      void handleMessagesStream(body, stream1, this.vokerClient);
      return stream2;
    });
  }
};
var handleMessagesStream = async (params, stream, vokerClient) => {
  const {
    vokerAgent,
    vokerAgentVersion,
    vokerPerson,
  ...
  const aggregator = new AnthropicMessagesStreamAggregator();
...
var Beta2 = class extends Anthropic.Beta {
  constructor(baseAnthropic, vokerClient) {
    super(baseAnthropic);
    this.vokerClient = vokerClient;
    this.messages = new BetaMessages(baseAnthropic, this.vokerClient);
  }
};
var BetaMessages = class extends Anthropic.Beta.Messages {
  constructor(baseAnthropic, vokerClient) {
    super(baseAnthropic);
    this.vokerClient = vokerClient;
  }
  create(params, options) {
    ...
    const messageRequest = super.create(restParams, options);
    if (params.stream === false || params.stream === null || params.stream === void 0) {
      return messageRequest.then((message) => {
        this.vokerClient.events.create({
          eventName: "llm",
          ...vokerParams,
    ...
    return messageRequest.then((response) => {
      ...
      const [stream1, stream2] = response.tee();
      void handleBetaMessagesStream(params, stream1, this.vokerClient);
      return stream2;
    });
  }
};
var handleBetaMessagesStream = async (params, stream, vokerClient) => {
  const {
    vokerAgent,
    vokerAgentVersion,
    vokerPerson,
  ...
  const aggregator = new AnthropicBetaMessagesStreamAggregator();
  for await (const chunk of stream) {
    aggregator.addEvent(chunk);
  }
  vokerClient.events.create({
...
var AnthropicMessagesStreamAggregator = class {
  current;
  constructor() {
    this.current = null;
  }
  addEvent(chunk) {
    this.current = this.accumulateMessage(chunk);
  }
  getResult() {
  ...
  // Modification of MessageStream.#addStreamEvent
...
var tracksToolInput3 = (content) => {
  return content.type === "tool_use" || content.type === "server_tool_use";
};
var betaTracksToolInput = (content) => {
  return content.type === "tool_use" || content.type === "server_tool_use" || content.type === "mcp_tool_use";
};
var JSON_BUF_PROPERTY3 = "__json_buf";
var AnthropicBetaMessagesStreamAggregator = class {
  current;
  constructor() {
    this.current = null;
  }
    ...
    switch (event.type) {
      ...
      case "content_block_delta": {
        ...
        switch (event.delta.type) {
          ...
          case "input_json_delta": {
            if (snapshotContent && betaTracksToolInput(snapshotContent)) {
              let jsonBuf = snapshotContent[JSON_BUF_PROPERTY3] || "";
              jsonBuf += event.delta.partial_json;
              const newContent = { ...snapshotContent };
              Object.defineProperty(
