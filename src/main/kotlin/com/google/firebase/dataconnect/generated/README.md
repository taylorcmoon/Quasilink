...
1 queries and
0 mutations,
a total of 1 operations.
Each of these operations is exposed
as a property of [ExampleConnector].


An example of the property for a query
is the query named "ListAIModels",
which can be accessed via the [ExampleConnector.listAiModels] property.
...
For example, the "ListAIModels" query has no variables
and can be executed via the
[ExampleConnector.listAiModels]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listAiModels.execute()
println("ListAIModels query returned: ${queryResult.data}")
```
